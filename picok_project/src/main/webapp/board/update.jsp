...
<body>
	...
	<div class="col-lg-5 m-auto">
		<form action="BoardUpdateController?board_idx=${singlepage.getBoard_idx()}" method="post"
			enctype="multipart/form-data">
			<!-- 내용이 여기서부터 시작됩니다. -->
			<div class="form-group" style="width: 100%; min-height: 200px; background-color: #F5F5F5; border-radius: 10px;">
					<img id="preview" src="/picok_project/upload/${singlepage.getFile_name()}"
					alt="미리보기"
					style="width: 100%; height: 100%; object-fit: cover;">
			</div>

			<div>
				<input type="file" name="file_name" onchange="photoView(event)"
					accept="image/*"><br /> <br/>
			</div>
			...
			<div class="form-group">
				<label for="subject">제목</label> <input type="text"
					class="form-control" id="board_title" name="board_title" value="${singlepage.getBoard_title()}"
					>
			</div>

			<div class="form-group">
				<label for="writer">작성자</label> <input type="text"
					value="${sessionScope.mvo.id}" class="form-control" id="writer"
			...
			<div>
				<input type="checkbox" hidden="hidden" name="secretOK" value="y"
					id="secretOK"> <label for="secretOK"></label>
			</div>
